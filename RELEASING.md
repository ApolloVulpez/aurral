...
The root `package.json` version is the single Aurral product version. The private frontend and backend workspace versions are not product versions.
...
Set the intended stable version on the working branch without creating a tag:

```bash
npm version 2.1.0 --no-git-tag-version --ignore-scripts
```

Commit the resulting `package.json` and `package-lock.json` changes with the feature.
...
2. Implement the change and set the intended version.
...
Create `hotfix/short-description` from `main`, use a `fix:` PR title, increment the patch version, and follow the same dev → test → PR → manual release flow. Do not patch a published tag or container in place.
