...
If Lidarr downloads and your player sees its library, mount that same media root into Aurral. This completes the filesystem setup.
...
3. If you use Navidrome, add Aurral's downloads path as a music folder.
4. If you use Plex, let Aurral register the downloads path.
...
You can also set `PATH_MAPPINGS=N:/ServerFolders/Music|/music` in the Compose file. Aurral applies it at runtime. Edit it in the UI afterward.
