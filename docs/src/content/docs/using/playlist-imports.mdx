...
- They keep the exact tracklist they were imported with.
...
- The active download pipeline may try multiple candidates and configured sources.
- After those options are exhausted, the track remains failed across worker and container restarts.
...
Static playlists and Lidarr are preferred when both exist. Multiple playlists may share one path; removing a playlist relocates survivors first so Navidrome does not see duplicates.
...
If Lidarr runs on Windows and Aurral runs in Docker, run **Test library access** in **Settings → Lidarr** to see the exact reported path, then add a mapping under **Settings → Download Clients → Remote Path Mappings**.
