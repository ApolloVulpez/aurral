...
1. Match Lidarr's media mount in Aurral and Navidrome ([Match Lidarr](/getting-started/storage/)).
2. In Navidrome, scan Lidarr's library **and** Aurral's Downloads Folder path (for example `/data/downloads/aurral`).
3. Connect Navidrome in **Settings → Playback → Navidrome**.
4. Let Aurral create or update its generated library and playlist files under `aurral-weekly-flow`.
...
This also occurs if its container mounts the same files at a different path. Playlists can appear, but tracks do not play.
...
Set **Downloads Folder → Path** to something under `/music` (for example `/music/Aurral`) in the UI.
...
:::caution
This setting also removes other missing Navidrome files and their ratings, play counts, and playlist entries. Use it only when missing files will not return.
:::
