...
This guide describes a self-hosted music stack. It uses Lidarr, Aurral, Navidrome, and Tailscale.
...
1. **Library albums**: Lidarr pulls full albums into `/data/music`. If necessary, use Tubifarry and slskd as the indexer and download client.
2. **Flows and playlists**: Aurral uses yt-dlp, slskd, or Usenet to download files into `aurral-weekly-flow/` under its Downloads Folder.
...
Do not treat all discovered music as a permanent collection. Each saved album needs storage, metadata, scans, backups, and maintenance.

Aurral keeps temporary music separate. It finds new artists, builds flows, imports playlists, and writes generated tracklists to its own folder.
...
- **Prowlarr + SABnzbd/NZBGet** for Usenet: Aurral uses Prowlarr to search indexers. It queues NZBs and imports completed files. See [Usenet](/integrations/usenet/).

You can run any combination and set **source priority** so Aurral falls back when one source fails.

### Same mount, different jobs

If Lidarr shares `/data` with slskd or Usenet, Aurral needs only that same mount:
...
7. Add full albums to Lidarr only when you want to keep them.
8. If necessary, use a [custom import list URL](/integrations/lidarr/#import-list-feeds) to send flow data to Lidarr.
...
| Library adjacent | Related picks connected to artists in your library         |
...
Static Aurral playlists are the memory layer. They preserve individual finds without making each find part of the core library.

For source mix, focus matching, and fallback behavior, see [Flows](/using/flows/).

## Storage, Navidrome, and file reuse

Aurral uses the Lidarr media mount. Keep the Lidarr library and Aurral-generated folders in separate paths under that root.

Separate paths make scans, cleanup, and backups clear.
...
Aurral reuses matching Lidarr, static-playlist, or flow files. It does not download these files again.

When you save a flow track to a static playlist, both playlists use the same file. Aurral moves the file when the flow refreshes.

Shared paths use one disk file. Thus, Navidrome does not show duplicates.
